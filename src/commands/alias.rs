    ...
    AliasCommands::List => {
      ...
      let get_upgrade_version = |version_str: &str| {
        let max_remote_version =
          find_max_matching_version(version_str, remote_versions.iter().map(String::as_str)).unwrap();

        let max_local_version =
          find_max_matching_version(version_str, local_versions.iter().map(String::as_str)).unwrap();
